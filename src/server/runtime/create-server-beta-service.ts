...
function buildServerGenerationProviderFromEnv(): ServerGenerationProvider | null {
  ...
  try {
    ...
    if (provider === 'openrouter') {
      ...
      if (!apiKey) return null;
      const opts: { apiKey: string; model?: string; baseUrl?: string; serviceTier?: string } = { apiKey };
      if (process.env.CLAUDE_MEM_SERVER_MODEL) opts.model = process.env.CLAUDE_MEM_SERVER_MODEL;
      ...
      const serviceTier = process.env.CLAUDE_MEM_SERVER_SERVICE_TIER ?? process.env.OPENAI_SERVICE_TIER;
      if (serviceTier) opts.serviceTier = serviceTier;
